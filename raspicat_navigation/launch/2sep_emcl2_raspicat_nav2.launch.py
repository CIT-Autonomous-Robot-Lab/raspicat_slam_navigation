...
def generate_launch_description():
    ...
    raspicat_nav_pkg_dir = get_package_share_directory('raspicat_navigation')
    ...
    declare_map_arg = DeclareLaunchArgument(
        'map',
        default_value='tsudanuma_2_19',
        description='Map name directory (e.g., tsudanuma_2_19) under <pkg>/config/map/'
    )

    # [修正] default_value が 'map' 引数 (map_arg) を参照するように変更
    declare_loc_map_yaml = DeclareLaunchArgument(
        'loc_map', 
        default_value=PathJoinSubstitution([
            raspicat_nav_pkg_dir, 'config', 'map', map_arg, 'localization', 
            PythonExpression(['"map_" + "', map_arg, '" + ".yaml"'])
        ]),
    ...
    declare_nav_map_yaml = DeclareLaunchArgument(
        'nav_map', 
        default_value=PathJoinSubstitution([
            raspicat_nav_pkg_dir, 'config', 'map', map_arg, 'navigation', 
            PythonExpression(['"map_" + "', map_arg, '" + ".yaml"'])
        ]),
